#!/usr/bin/env swift
// Renders the app icon into a .iconset, one PNG per size; `--dev` tints the mark rose and adds a Dev chip.
import AppKit
...
// Content outside the squircle makes macOS Tahoe plate the whole Dock icon on white.
