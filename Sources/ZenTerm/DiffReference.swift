...
enum DiffReference {
    /// `path:42-44` for a range, `path:42` for one line, bare `path` when there are no new-side lines
    /// to name (a deleted or binary file, or an empty selection). A selection of deletions only takes
    /// the line it follows, so the reference still lands in the right place in the file on disk.
