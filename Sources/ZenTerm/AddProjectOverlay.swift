...
final class AddProjectOverlay: NSView, ModalOverlay {
    ...
    private func validate(includeRequired: Bool) -> NSView? {
        ...
        let badEnvRow = envRows.first { $0.key.contains("\"") || $0.value.contains("\"") }
        envError.stringValue = badEnvRow == nil ? "" : "Variables can’t contain a \" character."
        envError.isHidden = (badEnvRow == nil)
        if let badEnvRow, firstInvalid == nil { firstInvalid = badEnvRow.keyBox.field }
...
// MARK: - Form controls (net-new; no editable-form primitives existed before)

/// A caption + control + inline validation message, stacked. The message sits directly beneath
/// its field and is hidden until `setMessage` gives it text.
