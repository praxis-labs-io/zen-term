...
struct ToolFloat: Equatable {
    ...
    /// authored and never stored in the config. It keys the live-float registry, the dock button map,
    /// and `toggle_float:<id>` keybinds, and it's how `ConfigWriter` finds a float's line. Identity
    ...
    let id: String
    /// Position in the dock, the palette, and Settings — the config `order:`, defaulting to the
