...
extension FileDiff {
    ...
    /// shared cache slot: a committed new-file has no old side, which would blank the old column
...
/// Per-repo syntax-highlight cache that outlives a single diff-viewer open. Owned by the
...
/// highlighted while the changed one re-parses. A reference type on purpose — the store is
/// shared, not copied.
final class DiffHighlightStore {
    ...
    /// content moved, so every unchanged file keeps its spans and repaints with no re-parse.
