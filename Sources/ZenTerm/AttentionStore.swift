...
final class AttentionStore {
    ...
    private var residual: [TabID: Residual] = [:]

    private struct Residual: Equatable {
        var state: SurfaceAttention
        var since: Date?
        var waiting: Int
        var waitingSince: Date?
    }
    ...
    func register(_ id: SurfaceID, tab: TabID?) {
        changing { entries[id] = Entry(tab: tab) }
    }
    ...
    private func changing(_ mutate: () -> Void) {
        guard depth == 0 else { return mutate() }
        let entriesBefore = entries
        let residualBefore = residual
        depth += 1
        mutate()
        depth -= 1
        if entries != entriesBefore || residual != residualBefore { onChange?() }
    }
