// --- CONFIGURATION ---
// zen-term: use the cursor color exactly as Ghostty passes it (sRGB, 0-1). The upstream shader ran
// it through an sRGB->linear conversion assuming a linear pipeline; ours isn't, so that darkened the
// trail below the cursor's own color. Raw keeps the trail the same color as the cursor.
vec4 TRAIL_COLOR = iCurrentCursorColor; // for a custom color: vec4(0.2, 0.6, 1.0, 0.5);
const float DURATION = 0.35; // total animation time (zen-term: tuned up from 0.2 for a Neovide-like lingering trail)
const float TRAIL_SIZE = 1.0; // 0.0 = all corners move together. 1.0 = max smear (leading corners jump instantly) (zen-term: max, was 0.8)
const float THRESHOLD_MIN_DISTANCE = 0.1; // min distance to show trail (units of cursor height) (zen-term: 0.1 so nearly every move smears, was 1.5)
