...
final class PaneCycleTests: WindowTestCase {
    ...
    /// Wrapping backwards off the front is the end nobody writes by hand, and `(i - 1) % n` is
    /// negative in Swift, so an unguarded modulo traps here rather than landing on the last pane.
    func test_prevPaneFromTheFirstPaneWrapsToTheLast() throws {
        let controller = makeWindow()
        controller.handle(.splitVertical)
        let first = try focused(controller)
        controller.handle(.nextPane)  // land on the other pane, then walk back past the front
        controller.handle(.nextPane)
        XCTAssertEqual(try focused(controller), first, "precondition: back at the first pane")

        controller.handle(.prevPane)

        XCTAssertNotEqual(try focused(controller), first)
        controller.handle(.nextPane)
        XCTAssertEqual(try focused(controller), first)
    }
